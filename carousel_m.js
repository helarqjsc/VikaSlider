function VikaSlider(obj) {
  ...
  instance.move = function (k, speed, ind) {
    if (instance.animate == false) {
      ...
      if (instance.pos >= instance.length) {
        $('#buttons-' + instance.id + ' .next').addClass('no');
      }
      if (instance.pos > 1) {
        $('#buttons-' + instance.id + ' .prev').removeClass('no');
      }
      ...
      if (instance._move == true) {
        $('#carousel-' + instance.id + ' .items').animate({
          'margin-left': -left
        }, speed, function () {
